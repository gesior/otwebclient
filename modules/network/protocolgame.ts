...
import {error, error, log} from "../log";
...
import {AwareRange, UnjustifiedPoints, Light} from "../structures";
...
export class ProtocolGame extends Protocol {
    ...
    parseMessage(msg: InputMessage) {
        ...
        try {
                ...
                */
                switch (opcode) {
                    ...
                    case Proto.GameServerChangeMapAwareRange:
                        ...
                        break;
                    default:
                        error("unhandled opcode %d", opcode);
                        break;
                }
                prevOpcode = opcode;
            }
            ...
            error("ProtocolGame parse message exception (%d bytes unread, last opcode is %d, prev opcode is %d): %s",
...
parseCompleteStorePurchase(msg: InputMessage)
{
    ...
    log("Purchase Complete: %s", message);
}
...
let entries = msg.getU8();
    ...
    log("Time %i, type %i, change %i, product name %s", time, productType, coinChange, productName);
}
}
...
parseStoreError(msg: InputMessage)
{
    ...
    let message = msg.getString();
    error("Store Error: %s [%i]", message, errorType);
}
...
parsePlayerHelpers(msg: InputMessage)
{
    ...
    if(creature)
        ...
        error("could not get creature with id %d", id);
}
...
parseMagicEffect(msg: InputMessage)
{
    ...
    if(!g_things.isValidDatId(effectId, ThingCategory.ThingCategoryEffect)) {
        error("invalid effect id %d", effectId);
        return;
    }
...
parseDistanceMissile(msg: InputMessage)
{
    ...
    if(!g_things.isValidDatId(shotId, ThingCategory.ThingCategoryMissile)) {
        error("invalid missile id %d", shotId);
        return;
    }
...
parseTalk(msg: InputMessage)
{
    ...
    let mode: MessageMode = Proto.translateMessageModeFromServer(msg.getU8());
    let channelId = 0;
    let pos: Position;
    ...
    switch(mode) {
        ...
        case MessageMode.MessageRVRChannel:
            ...
            break;
        default:
            error("unknown message mode %d", mode);
            break;
    }
...
{
    let code = msg.getU8();
    let mode: MessageMode = Proto.translateMessageModeFromServer(code);
    let text;
...
setTileDescription(msg: InputMessage, position: Position): number {
    ...
    for (let stackPos = 0; stackPos < 256; stackPos++) {
        ...
        if (stackPos > 10)
            error("too many things, pos=%s, stackpos=%d", position, stackPos);
...
getOutfit(msg: InputMessage): Outfit
{
    ...
    if(lookType != 0) {
        ...
        if(!g_things.isValidDatId(lookType, ThingCategory.ThingCategoryCreature)) {
            error("invalid outfit looktype %d", lookType);
            lookType = 0;
        }
        ...
        if(lookTypeEx == 0) {
            ...
            if(!g_things.isValidDatId(lookTypeEx, ThingCategory.ThingCategoryItem)) {
                error("invalid outfit looktypeex %d", lookTypeEx);
                lookTypeEx = 0;
            }
...
getMappedThing(msg: InputMessage): Thing
{
    ...
    if(x != 0xffff) {
        ...
        if(!thing)
            error("no thing at pos:%s, stackpos:%d", pos, stackpos);
    } else {
        let id = msg.getU32();
        thing = g_map.getCreatureById(id);
        if(!thing)
            error("no creature with id %u", id);
    }
...
getStaticText(msg: InputMessage, id: number): StaticText
{
    ...
    staticText: StaticText = StaticTextPtr(new StaticText);

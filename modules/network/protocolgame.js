...
export class ProtocolGame extends Protocol {
    ...
    parseAnimatedText(msg) {
        let position = this.getPosition(msg);
        let color = msg.getU8();
    ...
    parsePlayerSkills(msg) {
        ...
        for (let skill = 0; skill < lastSkill; skill++) {
            let level;
    ...
    parseTalk(msg) {
        ...
        if (g_game.getFeature(GameFeature.GameMessageLevel))
            level = msg.getU16();
        let mode = g_game.translateMessageModeFromServer(msg.getU8());
        let channelId = 0;
        let pos;
        switch (mode) {
    ...
    parseTextMessage(msg) {
        let code = msg.getU8();
        let mode = g_game.translateMessageModeFromServer(code);
        let text;
        switch (mode) {
            case MessageMode.MessageChannelManagement: {
    ...
    parseBotPackage(msg) {
        ...
        if (actionType == 6) {
            Log.log("botpackage throw", new Date(this.m_lastPacketTime).toISOString(), g_game.getLocalPlayer().getPosition(), mouseRightClickX, mouseRightClickY, mouseX, mouseY, gameX, gameY, gameW, gameH, isForeground, modulesCount, isHotkey);
        }
    }
    ...
    setMapDescription(msg, x, y, z, width, height) {
        let startz;
        let endz;
        let zstep;
        if (z > Otc.SEA_FLOOR) {
    ...
    getStaticText(msg, id) {
        let colorByte = msg.getU8();
        let color = Color.from8bit(colorByte);

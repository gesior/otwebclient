...
export class MapView {
    constructor() {
        ...
        this.m_drawDimension = new Size();
        this.m_visibleDimension = new Size();
        this.m_optimizedSize = new Size();
        this.m_virtualCenterOffset = new Point();
        this.m_visibleCenterOffset = new Point();
        this.m_moveOffset = new Point();
...
*/
        ...
        for (let iz = this.m_cachedLastVisibleFloor; iz >= this.m_cachedFirstVisibleFloor && !stop; --iz) {
            ...
            for (let diagonal = 0; diagonal < numDiagonals && !stop; ++diagonal) {
                ...
                for (let iy = diagonal - advance, ix = advance; iy >= 0 && ix < this.m_drawDimension.width() && !stop; --iy, ++ix) {
                    ...
                    let tilePos = cameraPosition.translated(ix - this.m_virtualCenterOffset.x, iy - this.m_virtualCenterOffset.y);
    ...
    updateGeometry(visibleDimension, optimizedSize) {
    ...
    lockFirstVisibleFloor(firstVisibleFloor) {
    ...
    setVisibleDimension(visibleDimension) {
